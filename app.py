...
Reasoning max 6 words. Exactly 2 analysis strings. All non-INSANE slips = exactly 5 legs.
...
def build_slip(api_key, stype, date_str, fixtures):
    ...
    fixture_lines = "\n".join(
        f"- {f['flag']} {f['time']} {f['home']} vs {f['away']} ({f['league']}) | Home: {f['homeWinPct']}% | Away: {f['awayWinPct']}% | Draw: {100-f['homeWinPct']-f['awayWinPct']}%"
        for f in fixtures
    )
            ...
            for i, (tab, slip) in enumerate(zip(tabs[:-1], slips)):
                with tab:
                    ...
                    for s in slip.get("selections",[]):
                        ...
                        odds_badge = f"<div style=\"background:#c0392b;border-radius:5px;padding:2px 7px;font-size:0.78rem;font-weight:800;color:white;margin-left:8px;\">{s.get('odds_est','')}x</div>" if is_ins and s.get('odds_est') else ""
                        st.markdown(f"""
                        <div style="display:flex;align-items:center;padding:9px 12px;border-radius:7px;
                                    ...
                                    border:1px solid {'rgba(192,57,43,0.3)' if is_ins else 'rgba(255,255,255,0.05)'};">
