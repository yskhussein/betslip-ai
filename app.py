...
SYSTEM_PROMPT = """You are an elite football betting analyst. Return ONLY raw JSON, no markdown.

RULES:
- Realistic outcomes only
- INSANE slip: exactly 12 legs, win-to-nils on weak sides, 50000x+ target
- All other slips: exactly 6 legs
- Eredivisie = 3.2 goals/game, use for goals picks
- Reasoning: max 8 words per leg
- Analysis: exactly 2 bullet points per slip
- Max 10 fixtures

JSON structure:
{"date":"","summary":"","fixtures":[{"time":"","league":"","flag":"","home":"","away":"","homeWinPct":0,"awayWinPct":0,"bestBet":""}],"slips":[{"type":"","title":"","subtitle":"","estimatedOdds":"","risk":"","riskColor":"green","legs":6,"selections":[{"num":1,"match":"","league":"","flag":"","selection":"","prob":"","reasoning":""}],"analysis":[""]}]}"""
...
def call_claude(api_key, date_str, leagues, slip_types):
    ...
    user_msg = f"Date: {date_str}\nLeagues: {', '.join(league_names)}\nSlips: {', '.join(slip_names)}\nGenerate compact JSON. 10 fixtures max. 6 legs per slip (12 for INSANE). 2 analysis points."
        ...
        for i in range(len(text)-1, 0, -1):
            if text[i] == '}':
                try:
                    ...
                    continue
        raise RuntimeError("Response was cut off. Try selecting fewer slip types.")
