...
def fetch_real_odds(odds_api_key, leagues, target_date):
    """Fetch real fixtures + odds from The Odds API for the selected leagues and date."""
    fixtures = []
    errors   = []
    ...
    for lid in leagues:
        ...
        try:
            ...
            if resp.status_code == 422:
                continue  # sport not available right now
            if resp.status_code != 200:
                raise RuntimeError(f"Odds API error {resp.status_code}")

            events = resp.json()
            for ev in events:
                # Filter by target date if possible
                ...
                # Convert decimal odds to rough win %
                def odds_to_pct(o):
                    if o and o > 1:
                        return round(100 / o)
                    return 0

                home_pct = odds_to_pct(home_odds)
                away_pct = odds_to_pct(away_odds)

                # Best bet suggestion
                if home_odds and away_odds:
                    if home_odds <= away_odds and home_odds <= (draw_odds or 99):
                        best_bet = f"Home Win ({home_odds})"
                    elif away_odds < home_odds and away_odds <= (draw_odds or 99):
                        best_bet = f"Away Win ({away_odds})"
                    else:
                        best_bet = f"Draw ({draw_odds})" if draw_odds else "Home Win"
                else:
                    best_bet = "Home Win"
...
def call_claude(api_key, date_str, leagues, slip_types, odds_api_key="", target_date=None):
    ...
    for stype in slip_types:
        ...
        try:
            ...
            real_note = (
                "IMPORTANT: These are REAL upcoming fixtures with real bookmaker odds. "
                "Use the actual team names, odds, and homeOdds/awayOdds/drawOdds fields "
                "to make realistic selections. Do NOT invent matches.\n"
                if using_real else ""
            )
