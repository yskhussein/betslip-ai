...
def fetch_real_odds(odds_api_key, leagues, target_date):
    ...
    def extract_markets(bookmakers, home, away, preferred_bm=None):
        """Pull 1X2, totals and btts from bookmaker list. Prefer preferred_bm if set."""
        result = {
        ...
        # Sort so preferred bookmaker comes first
        sorted_bms = sorted(
            bookmakers,
            key=lambda b: 0 if (preferred_bm and preferred_bm.lower() in b.get("key","").lower()) else 1
        )
        found_h2h = False
        for bm in sorted_bms:
            bm_key = bm.get("key","")
            is_toto = "toto" in bm_key.lower()
    ...
    for lid in leagues:
        ...
        try:
            # Request all three markets in one call; target Toto via bookmakers param first
            base_params = {
                "apiKey":     odds_api_key,
            ...
            # First attempt: Toto specifically
            toto_params  = {**base_params, "bookmakers": "toto", "regions": "eu"}
            resp = requests.get(
                f"https://api.the-odds-api.com/v4/sports/{sport_key}/odds/",
                params=toto_params, timeout=15
            )
            ...
            # If Toto returned nothing for this league, fall back to eu region
            if not events:
                eu_params = {**base_params, "regions": "eu"}
                resp2 = requests.get(
                    f"https://api.the-odds-api.com/v4/sports/{sport_key}/odds/",
                    params=eu_params, timeout=15
                )
            ...
            for ev in events:
                ...
                try:
                    ev_date = datetime.datetime.fromisoformat(commence.replace("Z","+00:00")).date()
                except Exception:
                    ev_date = None

                if ev_date and ev_date != target_date:
                    continue

                home = ev.get("home_team","")
                away = ev.get("away_team","")
                mkts = extract_markets(ev.get("bookmakers",[]), home, away, preferred_bm="toto")
                ...
                try:
                    t = datetime.datetime.fromisoformat(commence.replace("Z","+00:00"))
                ...
                fixtures.append({
                    ...
                    "bttsNo":      mkts["bttsNo"],
                    "oddsSource":  mkts["source"],   # "toto" or "eu"
                    "bestBet":     best_bet,
                })
...
def call_claude(api_key, date_str, leagues, slip_types, odds_api_key="", target_date=None):
    ...
    if using_real:
        # Use real fixtures; ask Claude only to generate the summary line
        fix_context   = real_fixtures
        date_display  = date_str
        summary_text  = f"Real fixtures from The Odds API · {len(real_fixtures)} matches found across {league_str}"
    else:
    ...
    if not st.session_state.result:
        ...
        if slips:
            ...
            with tabs[-1]:
                ...
                for f in fixtures:
                    src = f.get("oddsSource","")
                    src_badge = (
                        "<span style='background:#cc5500;color:white;font-size:0.65rem;font-weight:700;"
                        "border-radius:4px;padding:1px 5px;margin-left:6px;'>TOTO</span>"
                        if src == "toto" else
                        "<span style='background:#444;color:#aaa;font-size:0.65rem;font-weight:700;"
                        "border-radius:4px;padding:1px 5px;margin-left:6px;'>EU</span>"
                    )
                    over25   = f.get("over25")
